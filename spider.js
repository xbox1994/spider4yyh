...
function tMailShopDetailOptions(user_number_id){
        ...
        var req = https.request(tMailSearchResultOptions(keyword), function(res){
            ...
            res.on('end', function () {
                ...
                for(var i=0;i<$('.productShop-name').length; i++){
                    ...
                    var shopReq = https.request(tMailShopDetailOptions(shopNameId),function(res){
                        console.log(res.statusCode);
                        var shopHtmlContent = new BufferHelper();
                        res.on('data', function (chunk) {
                            shopHtmlContent.concat(chunk);
                        });
                        console.log(shopHtmlContent);

                    });
                    shopReq.end();
                    shopReq.on('error', function (err) {
                        console.log(err);
                    });
                }
            });
